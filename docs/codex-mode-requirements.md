...
| **回合级片段：`<current_time_reminder>`** | ✅ **prompt-align**（2026-08-15）：逐字标记 + `It is {UTC}.`；经快照 diff 按秒级刷新（codex 为限频注入，行为近似） |
...
### B.4 工具行为对照测试（2026-08-15 新增，fixture 提取自官方 HEAD 测试）

| 对照面 | fixture 来源 | 测试 | 状态 |
|--------|-------------|------|------|
| exec-policy 决策矩阵（safe/dangerous 名单） | `shell-command/src/command_safety/is_safe_command.rs` + `is_dangerous_command.rs` 官方单测 | `alignment/exec-policy.alignment.smoke.js`（172 safe + 44 dangerous 用例 × linux/win32） | ✅ ALL PASS。首跑发现 14 处不一致并修复：`;` 分隔捕获组 bug、`&&`/`\|\|`/`\|` 按子命令独立判定、括号/子 shell 拒绝、裸 `&` 拒绝、`--` 分隔符语义、`$(...)` 命令替换递归、`bash -c`/`sh -c` 嵌套递归、`if then` 语法错误不判危险 |
| exec_command 结果文本格式 | `core/src/tools/context.rs` `response_text()`/`truncated_output()`（442-468/412-440） | `alignment/exec-command.alignment.smoke.js`（6 用例） | ✅ ALL PASS。`renderExecResult` 重写对齐：`Wall time` 4 位小数、`Process exited with code`/`Process running with session ID`/`Original token count`/`Output:` 分段、截断警告前缀、信号死亡不渲染 exit-code 段 |
| apply_patch 输入输出（成功/失败路径） | `core/tests/suite/apply_patch_cli.rs` 集成测试（9 用例：多操作/双更新/Move/尾换行/插入/无效头/缺上下文/缺文件/删缺失） | `alignment/apply-patch.alignment.smoke.js` | ✅ ALL PASS。修复：输出按 A→M→D 分组（`print_summary` 语义）、验证失败统一加 `apply_patch verification failed:` 前缀 |
| 工具 schema 结构（exec_command/write_stdin） | `core/src/tools/handlers/shell_spec.rs`（属性名/必填/类型） | `alignment/tool-schema.alignment.smoke.js`（2 工具） | ✅ ALL PASS。修复：write_stdin 补 `max_output_tokens`。已知差异（文档化）：`additional_permissions`（DSH 无权限模型）、`environment_id`（单环境） |
